...
description: 'Human training data scaled by four orders of magnitude in six years and is now running dry — and even so it cannot reach AGI. The reason is what experience captures that human data cannot, and why generating new, verifiable knowledge requires acting, not reading.'
pubDate: '2026-06-09'
series: 'agi-qa'
...
Sutton's claim is not that human data is too small. It is that human data is the wrong *kind* of data, and that imitating it has a ceiling. A static corpus is a recording of people talking about the world, not access to the world; training on it can reach competence but, in his and Silver's words, "this approach in isolation has not and likely cannot achieve superhuman intelligence." The reason is structural: an agent that only imitates text is bounded by what its authors already knew, and "supervised learning does not involve experience." Even the rate of return is fading — "the pace of progress driven solely by supervised learning from human data is demonstrably slowing."
...
**Can synthetic data refill the well?**

Only when it is grounded — and grounded synthetic data is no longer "human data," it is experience. Resampling a model on its own output, with no external check, degrades it: Shumailov and colleagues show that "indiscriminate use of model-generated content in training causes irreversible defects in the resulting models, in which tails of the original content distribution disappear," an effect they name "model collapse." The rare, long-tail cases — exactly the part the human distribution is already thin on — vanish first.

The exception proves the rule about *what kind* of data matters. AlphaProof improved by generating its own data at scale because every generated proof was adjudicated by a formal verifier (the Lean proving system); DeepSeek-R1 developed reasoning because a correctness signal shaped it — "we simply provide it with the right incentives, and it autonomously develops advanced problem-solving strategies." In both, the generated data carried a consequence from an environment, not a model imitating itself. Silver and Sutton draw the line directly: "any static procedure for synthetically generating data will quickly become outstripped." Synthetic data rescues nothing on its own; synthetic data passed through a verifier is just experience under another name, which is the subject of Q2.
...
<p><strong>Reward</strong> — the scalar signal the environment returns, "what is good in an immediate sense" (Sutton &amp; Barto). It is handed to the agent by the world, not chosen by the learner.</p>
